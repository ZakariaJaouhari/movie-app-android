...
public class MyMovieAdapter extends
    ...
    private List<MyMovieData> originalMovieData = new ArrayList<>(); // Store original movie data
    private List<MyMovieData> filteredMovieData = new ArrayList<>(); // Store filtered movie data
    private Context context;

    public MyMovieAdapter(MyMovieData[] myMovieData, Context context) {
        this.originalMovieData.addAll(Arrays.asList(myMovieData));
        this.filteredMovieData.addAll(Arrays.asList(myMovieData));
        this.context = context;
    }

    public void addMovies(List<MyMovieData> newMovies) {
        int startPosition = filteredMovieData.size();
        this.originalMovieData.addAll(newMovies);
        this.filteredMovieData.addAll(newMovies);
    ...
    public void onBindViewHolder(@NonNull ViewHolder holder, int position) {
        final MyMovieData movieData = filteredMovieData.get(position); // Use filtered data
...
// Load image using Glide with TMDB URL
        Glide.with(context)
                .load("https://image.tmdb.org/t/p/w500" +
                        movieData.getMovieImage()) // Append TMDB base URL
    ...
    private Filter movieFilter = new Filter() {
        ...
        protected FilterResults performFiltering(CharSequence constraint) {
            ...
            if (constraint == null || constraint.length() == 0) {
                filteredList.addAll(originalMovieData); // If search query is empty, show all movies
            } else {
                String filterPattern =
                        constraint.toString().toLowerCase().trim();
                for (MyMovieData movie : originalMovieData) {
                    if
                    (movie.getMovieName().toLowerCase().contains(filterPattern)) {
                        filteredList.add(movie); // Add movie to filtered list if its name contains search query
                    }
                }
            }
