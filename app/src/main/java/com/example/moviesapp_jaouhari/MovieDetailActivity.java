...
import com.example.moviesapp_jaouhari.R;
...
// Retrieve movie ID from Intent extras
    ...
    private void fetchMovieDetails(int movieId) {
        String TMDB_API_KEY = "e21c9bd08ef733416fa4adc42dad2a14";
        ...
        JsonObjectRequest movieDetailsRequest = new JsonObjectRequest
                (Request.Method.GET, movieDetailsUrl, null, new
...
// Retrieve movie name, description, and image URL from the JSON response
                                    ...
                                    String imageUrl =
                                            "https://image.tmdb.org/t/p/w500" + response.getString("poster_path");
...
// Set error message in the TextView
        ...
        JsonObjectRequest movieVideosRequest = new JsonObjectRequest
                (Request.Method.GET, movieVideosUrl, null, new
                        Response.Listener<JSONObject>() {
                            @Override
                            public void onResponse(JSONObject response) {
                                try {
                                    if (response.has("results")) {
                                        ...
                                        for (int i = 0; i < results.length(); i++) {
                                            ...
                                            if
                                            (video.getString("type").equals("Trailer")) {
                                                trailerKey = video.getString("key");
// Log the trailer key for debugging
                                                Log.d(TAG, "Trailer Key: " +
                                                        trailerKey);
// Now you can use the trailer key as needed
// For example, you can store it in a member variable
// and use it later to play the trailer
// trailerKey = video.getString("key");
                                                break; // Assuming you only need the first trailer key
                                            }
                                        }
                                     }
                                } catch (JSONException e) {
...
// Construct the URL for the trailer
            String trailerUrl = "https://www.youtube.com/embed/" +
                    trailerKey;
...
// Show a message that the trailer is not available
            Toast.makeText(this, "Trailer not available",
...
// Move the camera to the current location with a zoom leve of 15
